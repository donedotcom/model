...
var objectFunctions = {
  ...
  _pathLeaf : function (path) {
    return path.split('.').pop();
  },
  
  _pathResource : function (path) {
    // `path` is assumed to be operating within the high-level
    // context of `this`, aka the model
  ...
  get : function (fieldPath) {
    var leaf = this._pathLeaf(fieldPath),
      resource = this._pathResource(fieldPath);

    if (!resource) {
      return null;
    }
    return this._wrapSchemaType(leaf, resource.attributes[leaf]);
  },

  set : function (pathHash) {
    var model = this;
    
    _.each(_.keys(pathHash), function (path) {
      var leaf = model._pathLeaf(path),
        resource = model._pathResource(path);
      
      if (resource) {
        resource.attributes[leaf] = pathHash[path];
      }
    });
    
  },

  // Set specified attribute values while leaving others alone
  ...
  /**
    Unset attribute (make undefined)
  */
